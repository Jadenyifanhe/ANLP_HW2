...
def predict(args, model, tokenizer):
    ...
    test_dataset, test_original = load_and_cache_examples(args, args.task_name, tokenizer, data_type='test')
    # Note that DistributedSampler samples randomly
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        if test_original[step].text_a[-1] == "TO_BE_CONTINUE":
            line.pop(-1)
            conll_results.append(line)
            need_reduce_flag = True
            pbar(step)
            continue
        if need_reduce_flag:
            need_reduce_flag = False
            conll_results[-1].extend(line)
        else:
            conll_results.append(line)
        pbar(step)
    logger.info("\n")
...
def main():
    ...
    if args.do_train:
        ...
        eval_dataset, _ = load_and_cache_examples(args, args.task_name, tokenizer, data_type='dev')
